...
const path = {
  ...
  src: {
    html: "src/*.html",
    scss: {
      intro: "./src/scss/pages/intro-styles.scss",
      corn: "./src/scss/pages/corn-harvesting.scss",
      calculator: "./src/scss/pages/calculator.scss",
      nitrogen: "./src/scss/pages/nitrogen.scss",
    },
    js: {
      corn: [
        "./src/js/corn/main.js",
      ...
      nitrogen: ["./src/js/nitrogen/nitrogen.js"],
    },
    img: "src/img/**/*.*",
    svg: "src/svg/**/*.svg",
...
export const html = () =>
  gulp
    ...
    .pipe(
      gulpif(!dev, htmlmin({ collapseWhitespace: true, removeComments: true })),
    )
...
export const scss = () => {
  const tasks = Object.entries(path.src.scss).map(([name, file]) =>
    gulp
      ...
      .pipe(
        compSass({ style: !dev ? "compressed" : "expanded" }).on(
          "error",
          compSass.logError,
        ),
      )
