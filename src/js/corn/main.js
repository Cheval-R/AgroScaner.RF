...
export async function main() {
  ...
  try {
    if (!byPeriod.checked) {
      ...
      if (!totalData)
        throw new Error(
          "Не удалось произвести расчёт, попробуйте позже или измените параметры",
        );
...
function GetDate(startDate, endDate) {
  ...
  const selectedYear = new Date(data.startDateMinus).getFullYear(),
  ...
  if (!byPeriod.checked) {
    if (selectedYear === currentYear) {
      ...
      if (
        new Date(data.startDateMinus).withoutTime().getTime() <
...
export function PrintResult(data) {
  ...
  if (clearData.date.length === 0 || clearData.temp.length === 0) {
    ...
    if (output) output.style.display = "block";
    if (outputToday)
      outputToday.textContent = "Недостаточно данных для расчёта.";
    if (outputOptimal) outputOptimal.textContent = "";
    return;
  }
...
function PrintEffectiveTemp(totalData, optimalHarvestingTiming, todayIndex) {
  ...
  if (todayIndex < 0 || totalData?.temp?.[todayIndex] == null) {
    ...
    if (outputToday) {
      outputToday.textContent =
        "Фактических данных на сегодняшний день нет. Используется прогноз.";
    }
  } else {
